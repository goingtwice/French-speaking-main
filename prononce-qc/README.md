...
A pronunciation trainer for people sitting **TEF Canada / TCF Canada**.

Type an English or French word and hear real humans say it — a Québécois speaker
next to a Parisian one next to a Belgian one — with the IPA for each, at any
speed, on a loop.

No backend. No build step. No account. One static folder you can drop on GitHub
Pages and open from your phone.

---

## Why it works this way

The problem is usually framed as *"I can't find a source with the correct French
...
56 checks: the IPA derivation, the Wiktionary template parser, region
classification, and a full pass through the UI in Chromium with the network
stubbed against realistic wikitext fixtures. Writes three screenshots to
`test/`.
...
were correct at time of writing, but **IRCC changes thresholds and TEF has
changed its scale before**. The app links straight to the official page. Confirm
there before you book anything.
...
data.js                 exam tables, Québec rules, reduced forms,
                        minimal pairs, ~90 vocab entries, phrases
app.js                  everything else
worker/forvo-proxy.js   optional Cloudflare Worker
test/e2e.mjs            Playwright suite
