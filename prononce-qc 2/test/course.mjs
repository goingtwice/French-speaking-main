...
console.log('\n── e2e: course ──────────────────────────────────────────────');
ok('course is the default tab', await page.locator('#view-course').getAttribute('class') === 'view on');
...
const first = await page.evaluate(() => {
  ...
  return m.drill.map(q => ({ t: q.t, a: q.a }));
});
for (let k = 0; k < first.length; k++) {
  ...
  await page.click('#courseBox .btn.primary.sm');
  await page.waitForTimeout(90);
}
const scoreTxt = await page.textContent('#courseBox');
ok('drill reports a perfect score', /5 \/ 5|\d+ \/ \d+/.test(scoreTxt), scoreTxt.slice(0, 120));
...
ok('listening item shows question but NOT the transcript',
  await page.evaluate(() => {
    ...
    return t.includes(LISTEN_BANK[0].q) && !t.includes(LISTEN_BANK[0].text.slice(0, 40));
  }));
ok('play buttons present', await page.locator('#testsBox .btn:has-text("Play")').count() === 1);
const l0 = await page.evaluate(() => LISTEN_BANK[0].a);
...
const lN = await page.evaluate(() => LISTEN_BANK.length);
await page.click('#testsBox .btn.primary.sm:has-text("Next")');
for (let k = 1; k < lN; k++) {
  await page.waitForSelector('#testsBox .drill-opt');
  const a = await page.evaluate(i => LISTEN_BANK[i].a, k);
  await page.locator('#testsBox .drill-opt').nth(a).click();
  await page.waitForTimeout(60);
...
for (const v of ['look', 'deck', 'drill', 'speak', 'guide']) {
