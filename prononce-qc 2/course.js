...
function drillView(m) {
  ...
  function paint() {
    box.innerHTML = '';
    if (i >= m.drill.length) {
      const pct = Math.round(100 * ok / m.drill.length);
      S.course.drill[m.id] = { ok, n: m.drill.length };
      if (pct >= 70) S.course.done[m.id] = true;
      save();
      box.append(el('h2', {}, `${ok} / ${m.drill.length}`),
    ...
    box.append(el('div', { class: 'between', style: 'margin-bottom:10px' },
      el('span', { class: 'score' }, `${i + 1} / ${m.drill.length}`),
    ...
    const settle = (right, extra) => {
      ...
      after.append(el('div', { class: 'row' }, el('div', { class: 'spacer' }),
        el('button', { class: 'btn primary sm', onclick: () => { i++; paint(); } },
          i + 1 >= m.drill.length ? 'See score →' : 'Next →')));
    };
...
function renderTests() {
  ...
  if (panel) {
    ...
    const fn = { place: panelPlacement, listen: panelListen, read: panelRead,
                 write: panelWrite, speak: panelSpeak, tmpl: panelTemplates, plan: panelPlan }[panel];
    if (fn) box.append(fn());
    return;
  }
  ...
  const cards = [
    { k: 'listen', t: 'Listening practice', d: `${LISTEN_BANK.length} items read aloud by your device voice, across A1 to B2. Transcript hidden until you answer.` },
    { k: 'read',   t: 'Reading practice',   d: `${READ_BANK.length} passages in exam format — notices, letters, articles, argument texts.` },
...
function panelListen() {
  const wrap = el('div');
  let pool = LISTEN_BANK.slice(), i = 0, ok = 0;
  ...
  function paint() {
    ...
    if (i >= pool.length) {
      ...
      box.append(el('h2', {}, `${ok} / ${pool.length}`),
        ...
        el('button', { class: 'btn primary sm', onclick: () => { i = 0; ok = 0; pool = LISTEN_BANK.slice().sort(() => Math.random() - .5); paint(); } }, 'Again, shuffled'));
      return;
    }
    const it = pool[i];
...
function panelRead() {
  ...
  function paint() {
    box.innerHTML = '';
    if (i >= READ_BANK.length) {
      const n = nclcFromPct(Math.round(100 * ok / READ_BANK.length));
      logResult('Reading practice', ok, READ_BANK.length);
      box.append(el('h2', {}, `${ok} / ${READ_BANK.length}`),
        el('p', { class: 'sub' }, 'Rough indication: NCLC ' + n + '.'),
        el('div', { class: 'note warn' }, BANDS.note),
        el('button', { class: 'btn primary sm', onclick: () => { i = 0; ok = 0; paint(); } }, 'Again'));
      return;
    }
    const it = READ_BANK[i];
    box.append(el('div', { class: 'between', style: 'margin-bottom:12px' },
      el('span', { class: 'score' }, `${i + 1} / ${READ_BANK.length}`),
...
window.__pqc = { judge, norm, nclcFromPct, MODULES, showPanel };
